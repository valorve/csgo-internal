...
static void studio_align_ik_matrix(matrix3x4_t& mMat, const vec3d& vAlignTo) {
	vec3d tmp1, tmp2, tmp3;

	tmp1 = vAlignTo;
	tmp1 = tmp1.normalize_in_place();
	...
	tmp2 = tmp3.cross_product(tmp1);
	tmp2 = tmp2.normalize_in_place();

	mMat[0][1] = tmp2.x;
	mMat[1][1] = tmp2.y;
	mMat[2][1] = tmp2.z;

	tmp3 = tmp1.cross_product(tmp2);
...
static bool studio_solve_ik(int thigh, int knee, int foot, vec3d& target_foot, vec3d& target_knee_pos, vec3d& target_knee_dir, matrix3x4_t* bone_to_world) {
	...
	if (ik_foot.length() > (l1 + l2) * KNEEMAX_EPSILON) {
		ik_foot = ik_foot.normalize_in_place();
	...
	if (ik_foot.length() < min_distance) {
		ik_foot = (world_foot - world_thigh).normalize_in_place();
		ik_foot *= min_distance;
	}

	c_ik_solver ik;
	if (ik.solve(l1, l2, (float*)&ik_foot, (float*)&ik_traget_knee, (float*)&ik_knee)) {
		...
		matrix_world_knee[0][3] = ik_knee.x + world_thigh.x;
		matrix_world_knee[1][3] = ik_knee.y + world_thigh.y;
		matrix_world_knee[2][3] = ik_knee.z + world_thigh.z;

		matrix_world_foot[0][3] = ik_foot.x + world_thigh.x;
		matrix_world_foot[1][3] = ik_foot.y + world_thigh.y;
		matrix_world_foot[2][3] = ik_foot.z + world_thigh.z;
...
static bool studio_solve_ik(int thigh, int knee, int foot, vec3d& target_foot, matrix3x4_t* bone_to_world) {
	auto world_foot = bone_to_world[foot].get_origin();
	auto world_knee = bone_to_world[knee].get_origin();
	auto world_thigh = bone_to_world[thigh].get_origin();
	...
	auto ik_knee_dir = (ik_knee - ik_half).normalize_in_place();
